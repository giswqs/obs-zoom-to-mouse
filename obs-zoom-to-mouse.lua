...
---
-- Get all valid display capture source IDs for the current platform
---@return table
function get_valid_dc_source_ids()
    local source_ids = {}
    
    if ffi.os == "Windows" then
        table.insert(source_ids, "monitor_capture")
    elseif ffi.os == "Linux" then
        table.insert(source_ids, "xshm_input")
    elseif ffi.os == "OSX" then
        -- Include both old and new macOS source types
        table.insert(source_ids, "screen_capture")     -- New macOS (OBS 30+)
        table.insert(source_ids, "display_capture")    -- Old macOS (OBS <30)
        table.insert(source_ids, "av_capture_input")   -- Alternative macOS capture
    end
...
function is_display_capture(source_to_check)
    if source_to_check ~= nil then
        local valid_source_ids = get_valid_dc_source_ids()
        local source_type = obs.obs_source_get_id(source_to_check)
        
        -- Check if this source type is in our list of valid display capture sources
        for _, valid_id in ipairs(valid_source_ids) do
            if source_type == valid_id then
                return true
            end
        end
        
        -- If allow_all_sources is enabled, accept any source as valid
...
function populate_zoom_sources(list)
    ...
    if sources ~= nil then
        ...
        for _, source in ipairs(sources) do
            ...
            -- Check if this source type is valid for display capture
            for _, valid_id in ipairs(valid_source_ids) do
                if source_type == valid_id then
                    is_valid = true
                    break
            ...
            if is_valid then
                ...
                log("Added source: " .. name .. " (type: " .. source_type .. ")")
            end
        end
